	...
	class ReduxFramework_extension_metaboxes {
		...
		public function __construct( $parent ) {
			...
			if ( empty( self::$_extension_dir ) ) {
				...
				$this->_extension_dir = trailingslashit( str_replace( '\\', '/', plugin_dir_path( __FILE__ ) ) );
...
// Copyright Dovy Paukstys (@dovy) of Redux Framework
					...
					foreach ( $box['post_types'] as $posttype ) {
						...
						if ( isset( $box['title'] ) ) {
							...
							if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
								...
								$title = ucfirst( $posttype ) . " " . esc_html__( 'Options', 'startup-pro' );
		...
		function generate_boxes( $post, $metabox ) {
			...
			<div
				...
				<div class="redux-notices">
					...
					<?php if ( $sidebar ) : ?>
						...
						<div class="saved_notice admin-notice notice-blue" style="display:none;">
							...
							<strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'startup-pro' ) ) ?></strong>
						...
						<div class="redux-save-warn notice-yellow">
							...
							<strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'startup-pro' ) ) ?></strong>
					...
					<div class="redux-field-errors notice-red">
						...
						<strong><span></span> <?php echo esc_html__( 'error(s) were found!', 'startup-pro' ) ?></strong>
					...
					<div class="redux-field-warnings notice-yellow">
						...
						<strong><span></span> <?php echo esc_html__( 'warning(s) were found!', 'startup-pro' ) ?>
				...
				echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . esc_html__( 'Expand', 'startup-pro' ) . '</a>';
		...
		function meta_boxes_show_errors() {
			...
			if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
				...
				echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . esc_html__( 'error(s) were found!', 'startup-pro' ) . '</strong></p>';
			...
			if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
				...
				echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . esc_html__( 'warnings(s) were found!', 'startup-pro' ) . '</strong></p>';
...
if ( ! function_exists( 'smart_redux_post_meta' ) ) {
	function smart_redux_post_meta( $opt_name = "", $thePost = array(), $meta_key = "", $def_val = "" ) {
		global $post;
		if ( empty( $opt_name ) ) {
			return;
		}
