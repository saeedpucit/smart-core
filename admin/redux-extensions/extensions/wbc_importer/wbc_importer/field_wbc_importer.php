            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'startup-pro' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'startup-pro' );
                    }
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'startup-pro' ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'startup-pro' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'startup-pro' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'startup-pro' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'startup-pro' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'startup-pro' )."</h5>";
            }
