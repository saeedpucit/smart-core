...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
                        ...
                        echo esc_html__( "Demo Already Imported", 'startup-pro' );
                    }
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'startup-pro' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'startup-pro' );
            $check = $this->check_php_ini_variables();
            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'.__("*NOTE: If you import demo content. It will overwrite the existing data and settings. It's not included revolution slider and Essential Grid. So you have to configure that settings once it's done.", 'startup-pro' ).'</p><p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'startup-pro' ) ).'</p><p>'.$check.'</p>',
        ...
        function check_php_ini_variables()
        {
            ...
            */
            $val = '';
            $val .= '<strong>'.__('System Status:', 'startup-pro' ).'</strong><br>';
            if(ini_get('max_execution_time') < 300 ){
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-exclamation"></i></span><span class="pro-check-phpini-bad">max_execution_time = ' . ini_get('max_execution_time') . ' '. __('*RECOMMENDED 300', 'startup-pro' ).'</span><br>';
            }
            else {
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-check"></i></span><span class="pro-check-phpini-good">max_execution_time = ' . ini_get('max_execution_time') . "</span><br>";
            }

            if(substr_replace(ini_get('memory_limit'),"",-1)  < 264 ){
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-exclamation"></i></span><span class="pro-check-phpini-bad">memory_limit = ' . ini_get('memory_limit')  . ' '. __('*RECOMMENDED 512M', 'startup-pro' ).'</span><br>';
            }
            else {
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-check"></i></span><span class="pro-check-phpini-good">memory_limit = ' . ini_get('memory_limit')  . "</span><br>";
            }

            if(substr_replace(ini_get('post_max_size'),"",-1)  < 32 ){
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-exclamation"></i></span><span class="pro-check-phpini-bad">post_max_size = ' . ini_get('post_max_size')  . ' '. __('*RECOMMENDED 64M', 'startup-pro' ).'</span><br>';
            }
            else {
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-check"></i></span><span class="pro-check-phpini-good">post_max_size = ' . ini_get('post_max_size')  . "</span><br>";
            }

            if(substr_replace(ini_get('upload_max_filesize'),"",-1)  < 32 ){
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-exclamation"></i></span><span class="pro-check-phpini-bad">upload_max_filesize = ' . ini_get('upload_max_filesize')  . ' '. __('*RECOMMENDED 64M', 'startup-pro' ).'</span><br>';
            }
            else {
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-check"></i></span><span class="pro-check-phpini-good">upload_max_filesize = ' . ini_get('upload_max_filesize')  . "</span><br>";
            }

            if(ini_get('max_input_vars') < 2000 ){
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-exclamation"></i></span><span class="pro-check-phpini-bad">max_input_vars = ' . ini_get('max_input_vars') . ' '. __('*RECOMMENDED 2000', 'startup-pro' ).'</span><br>';
            }
            else {
                $val .= '<span class="pro-check-phpini-icon"><i class="fa fa-check"></i></span><span class="pro-check-phpini-good">max_input_vars = ' . ini_get('max_input_vars') . "</span><br>";
            }
