        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    ...
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'startup-pro' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'startup-pro' ) . '</a> v' . self::$_version ) . '</span>';
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    ...
                    $this->args['menu_title'] = __( 'Options', 'startup-pro' );
                ...
                if ( empty ( $this->args['page_title'] ) ) {
                    ...
                    $this->args['page_title'] = __( 'Options', 'startup-pro' );
            ...
            private function _internationalization() {
                 ...
                 * @param string     'startup-pro'  text domain

                 */

                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'startup-pro' );
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    ...
                    load_plugin_textdomain( 'startup-pro', '', self::$_dir . 'languages/' . $locale . '.mo' );

                } else {

                    load_textdomain( 'startup-pro', self::$_dir . 'languages/' . $locale . '.mo' );
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'startup-pro' );

                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {

                        $event = __( 'clicking', 'startup-pro' );
                    ...
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'startup-pro' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    ...
                    $default_output = __( 'Default', 'startup-pro' ) . ": " . substr( $default_output, 0, - 2 );
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    ...
                    echo json_encode( array(
                        ...
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'startup-pro' ),
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    ...
                    echo json_encode( array(
                        ...
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'startup-pro' ),
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'startup-pro' ) ) );
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                ...
                                foreach($arr as $x => $y) {
                                    ...
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        ...
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'startup-pro');
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                ...
                                foreach($arr as $x => $y) {
                                    ...
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        ...
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'startup-pro');
